...
public class KhuyenMaiBoardGUI extends JPanel {
	
	private int indexNavigationItemCache;
	private JPanel contentPanel;
	private	KhuyenMaiFormPanel formPanel;
	private KhuyenMaiTablePanel tablePanel;
	private String formatUrlIcons;

	public KhuyenMaiBoardGUI(Dimension size) {
		setSize(size);
		setLayout(null);
		initVariable();
		initComponent();
	...
	private void initVariable() {
		...
		formPanel = new KhuyenMaiFormPanel();
		tablePanel = new KhuyenMaiTablePanel();
	...
	private void initComponent() {
		...
		// ================== LABEL TITLE =======================
		JLabel labelTitleBoard = new JLabel("Khuyến mãi");
	...
	private void setContentNavigationBar(JPanel navigationPanel) {
		...
		String[] buttonTexts = {"Danh sách khuyến mãi", "Thêm khuyến mãi"};
	...
	private void EventClickNavigationButton(int indexButton) {
		if(indexButton == 0) {
			...
			formPanel.setVisible(false);
			tablePanel.refreshKhuyenMaiTable();
			//tablePanel.resetTable(formPanel.wasAddBill());
		}
		if(indexButton == 1) {
